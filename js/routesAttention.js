...
function rutesAttentionMovil(json) {
  ...
  $(".arrows-lienzo").on("click", function (event) {
    ...
    if ($("body").hasClass("active")) {
      ...
      if (
        valiate_params_url() !== "1111" &&
        $("header").hasClass("active-url")
      ) {
      ...
      json.then(function (value) {
        ...
        if (
          ...
          $("#arrows-1").attr(
            "title",
            dimension + "," + attetionAux + "," + entity + "," + name
          );
        } else {
    ...
    const timer = setInterval(function () {
      ...
      if (sizeScreen < sizeScreenWidth * timeMultiplier) {
        $(`${clasName}`).css(
          ...
          "" + increase + "% , " + increasefg2 + "%," + increasefg2 + "%"
        );
        increase += 1;
        sizeScreen += 10;
        increasefg2 += 2;
      ...
      if (sizeScreen < sizeScreenWidth * timeMultiplier) {
        ...
        if (sizeScreen <= timeHideRute) {
          ...
          if (bandAux) {
            ...
            show_brecumbs();
            $("#arrow-box-principality").removeClass(
              "breadcrumbs-active-click"
            );
          }
    ...
    .then(function (value) {
      ...
      var animateCharac = setInterval(function () {
        if (sizeScreen < sizeScreenWidth * timeMultiplier) {
          ...
          if (timeSequence == 4) {
            if (imgSequence - 1 == 0) {
              $(`#personaje-caminata-${imgSequence + 7}`).addClass(
                "features-personajes-off"
              );
            }
            $(`#personaje-caminata-${imgSequence - 1}`).addClass(
              "features-personajes-off"
            );
            $(`#personaje-caminata-${imgSequence}`).removeClass(
              "features-personajes-off"
            );
...
function funcionalityRute(
  ...
  json
    .then(function (value) {
      ...
      try {
        objectJson = [
          Object.entries(Object.values(value)[dimension].TiposDeAtencion),
          Object.entries(
            Object.entries(Object.values(value)[dimension].TiposDeAtencion)[
              attetionAux
            ][1].sedes
        ...
        console.log(
          "la instancia del json selecionada, esta vacia o le faltan componentes",
          err
        );
        objectJson = [
          Object.entries(Object.values(value)[dimension].TiposDeAtencion),
      ...
      for (const attention of objectJson[indexCap]) {
        ...
        funcionality_title_text(
          nameArrow,
          iterador,
          attention[1],
          attention[1].tipoTitulo
        );
    ...
    .catch((err) => {
      console.log("ups, paso un error al insertar el texto 1", err);
    });
}
...
function funcionalityAskDetoting(index) {
  ...
  $(".detonating-question-box")
    .find(".detonating-question-text")
    .html(" ¿Qué tipo de " + text[index] + " estás buscando?");
...
function funcionalityInfoContent(json, dimension, attetionAux, entity, index) {
  json
    .then(function (value) {
      ...
      $(".title-content-info")
        ...
        .html(
          Object.values(
            Object.entries(
              Object.values(value)[dimension].TiposDeAtencion[attetionAux]
                .sedes[entity].Entidades
            )[index][1].Atenciones
          )[0].name
        );
    ...
    .catch((err) => {
      console.log("ups, paso un error al insertar el texto 2", err);
    });
}

function titleBrecumbs(indexArr, indexCap) {
  //insertar el texto en brecumbs
  textBrecumbs = $("#arrows-" + indexArr + "")
    .find("text")
    .text();
  $("#bread-" + (indexCap + 1) + "")
    .find("span")
    .html(textBrecumbs.slice(0, 9));
}
...
function funcionality_botton_text(json, dimension, attetionAux, entity, index) {
  ...
  $(".boton-info").on("click", function (event) {
    ...
    json
      .then(function (value) {
        //insertar el contenido
        $(".content-text-botton-box")
          .find(".content-text-botton")
          .html(
            Object.values(
              Object.entries(
                Object.values(value)[dimension].TiposDeAtencion[attetionAux]
        ...
        if ($(botonPress).attr("id") === "enlaces") {
          ...
          $(".content-text-botton-box")
            .find(".content-text-botton")
            .html('<ul style="color:gray">' + textEnlace + "</ul>");
          textEnlace = "";
        }
      })
      .catch((err) => {
        console.log("ups, paso un error al insertar el texto 3", err);
      });
...
function create_box_botton(json, dimension, attetionAux, entity, typeBox) {
  ...
  json
    .then((value) => {
      ...
      textKeys = Object.keys(
        Object.values(
          Object.entries(
            Object.values(value)[dimension].TiposDeAtencion[attetionAux].sedes[
              entity
            ].Entidades
          )[typeBox][1].Atenciones
        )[0]
      );
      //obtiene el contenido de las sedes
      textValues = Object.values(
        Object.values(
          Object.entries(
            Object.values(value)[dimension].TiposDeAtencion[attetionAux].sedes[
              entity
      ...
      for (const nameSedes of textKeys) {
        if (iterador > 6) {
          $(".box-button-dimension-content").append(
            '<div class="botton-sedes">' + nameSedes + "</div>"
          );
        }
        iterador++;
      }
        ...
        if (!$(click).hasClass("botton-dimension-active")) {
          ...
          $(".botton-sedes").each(function (indexAux, value) {
            ...
            if (index == indexAux && indexAux != 8) {
              ...
              index == 8
                ? ((gridPorcent += "15% 85%"), (bottonClick = value))
                : (gridPorcent += "15%");
              $(".box-button-dimension-content").css(
                "grid-template-rows",
                gridPorcent
              );
              $(bottonClick).after(
                '<div class="text-button-dimension" id="1"> ' +
                  textValues[index + 7] +
                  " </div>"
              );
            } else {
    ...
    .catch((err) => {
      console.log("ups, paso un error al insertar el texto 4", err);
    });
}
      ...
      $(".button-abc").on("click", function (event) {
        ...
        if ($(numButtons[0]).attr("class") === "box-button-abc-aux1") {
          ...
          $("#text-abc-content").html(
            ...
            )[0][accionAbc[indexAux]]
          );
        }
      });
    })
    .catch((err) => {
      console.log("ups, paso un error al insertar el texto 5", err);
    });
}
...
function dimension_description(json, dimension) {
  ...
  json
    .then((event) => {
      $(".title-dimension .title-aux-dimensio-content").html(
        `Dimension ${Object.values(event)[dimension].name.replace(",", " ")}`
      );
      $(".content-dimension").html(
        Object.values(event)[dimension].introduccion
      );
    })
    .catch((err) => {
      console.log("ups, paso un error al insertar el texto 6", err);
    });
}
...
function event_click_div() {
  ...
  $(".box-x-dimension-aux , .box-x-dimension-end-aux").on(
    "click",
    function (event) {
...
function funcioanlity_box_escritorio(
  json,
  dimension,
  attetionAux,
  entity,
  ...
  json
    .then(function (value) {
      ...
      //$('.overlay').show()
      $(".title-escritorio-dimension").html(
        Object.values(
          Object.entries(
            Object.values(value)[dimension].TiposDeAtencion[attetionAux].sedes[
              entity
    ...
    .catch((err) => {
      console.log("ups, paso un error al insertar el texto 7", err);
    });
}
...
function get_time_multiplier(value, dimension, attetionAux, entity, indexCap) {
  ...
  //30% of right
    ...
    (objectJson[indexCap].length > 5 &&
      sizeScreenWidth11 <= 375 &&
      sizeScreenHeigth <= 667) ||
    (objectJson[indexCap].length > 5 &&
      sizeScreenWidth11 <= 500 &&
      sizeScreenWidth11 > 375 &&
...
function get_increase_size(
  ...
  //30% of right
  if (
    (sizeScreenWidth11 <= 375 && sizeScreenHeigth <= 667) ||
    (sizeScreenWidth11 <= 500 &&
      sizeScreenWidth11 > 375 &&
      sizeScreenHeigth <= 800)
  ) {
  ...
  json
    .then((value) => {
      ...
      if (params_boolean !== "1111") {
        ...
        var permission_walk = get_name_dimension(
          value,
          array_value_params[0],
          array_value_params[1],
          array_value_params[2],
          array_value_params[3],
          index - 1,
          "caminata"
        );
        //las capas de la dimension inician en 1 por eso se resta 1
        show_breadcrumb = permission_walk
          ? index - 1
          : index === 4
          ? index - 1
          : index - 2;
        ...
        $("body").attr(
          "title",
          index < 4 ? (permission_walk ? index : index - 1) : 3
        );
        $("#bread-1").removeClass("box-breadcrumbs-active");
        $("#arrows-1").attr(
          "title",
        ...
        for (
          var i = 0;
          i < (!permission_walk && index != 4 ? index - 1 : index);
          i++
        ) {
          console.log("i", i);
          name_dimension = get_name_dimension(
            value,
            array_value_params[0],
            array_value_params[1],
            array_value_params[2],
          ...
          funcionalityBrecumbs(
            ...
            name_dimension
              .split(",")
              [name_dimension.split(",").length - 1].replace(",", " ")
              .toUpperCase()
          );
          show_breadcrumb--;
        }
        ...
        if (index < 4) {
          if (
            size_object_senalitica(
              value,
              array_value_params,
              permission_walk ? index - 1 : index - 2
            ).length > 5
          ) {
            $(`.lienzo`).css(
              "right",
        ...
        add_class_active_brecumbs_params_url(
          !permission_walk && index != 4 ? index - 1 : index
        );
        ...
        funcionalityRute(
          ...
          array_value_params[2],
          1,
          index < 4 ? (permission_walk ? index - 1 : index - 2) : 2,
          "arrows-"
        );
...
function get_name_dimension(
  ...
  objectCap = {
    ...
    1: Object.values(Object.values(value)[index].TiposDeAtencion)[attention][
      key
    ],
    2: Object.values(
      Object.values(Object.values(value)[index].TiposDeAtencion)[attention]
        .sedes
    )[sedes][key],
    3: Object.values(
      Object.values(
 ...
 * @returns
 */
function get_info_box_senalitica(
  value,
  dimension,
  attention,
  entity,
